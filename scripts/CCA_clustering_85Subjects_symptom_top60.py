...
hierarchy.dendrogram(Z, color_threshold=6, above_threshold_color='grey')
 
# Add horizontal line.
plt.axhline(y=7.3, c='black', lw=2, linestyle='dashed')
...
for i in range(datat.shape[0]):
    ax.scatter(datat[i,0], datat[i,1], c = color[ac2.fit_predict(X_new)[i]], cmap ='rainbow')#,label=labels[ac2.fit_predict(X_new)[i]]) 
...
'''
top20=['Rinsula','Rpostcentral','N_Right-Amygdala','Ltemporalpole',
       'Rmiddletemporal','Rprecuneus','Lpostcentral','Lrostralanteriorcingulate',
       'Rparahippocampal','N_Left-Hippocampus','Lparahippocampal','Rmedialorbitofrontal',
       'Lsuperiorparietal','Lparstriangularis','N_Left-Pallidum','Ltransversetemporal',
       'Lfusiform','N_Right-Pallidum','N_Right-VentralDC','N_Left-choroid-plexus'
       ]
'''
